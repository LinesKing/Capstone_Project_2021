function plotConvexRegion(A,b,lb,ub,c,transp,points,linetyp,start_end)
%%%% The function plots closed convex regions in 2D/3D. The region is formed
%%%% by the matrix A and the vectors lb and ub such that Ax>=b and lb<=x<=ub,
%%%% where the region is the set of all feasible x (in R^2 or R^3).
...
%%%%    transp - (optional) A measure of how transparent the region will 
%%%%             be. Must be between 0 and 1. Default is 0.5.
%%%%    points - (optional) Points in matrix form.
%%%%    linetyp - (optional) How the points will be marked.
%%%%              Default is 'k-'.
%%%%    start_end - (optional) If a special marking for the first and last 
%%%%                point is needed.
%%%% Output:
%%%%    NA
%%%%
%%%% If several regions must be plotted, A ,b, lb, ub and c can be stored 
%%%% as a cell array {} containing all sub-set information. 
%%%%
