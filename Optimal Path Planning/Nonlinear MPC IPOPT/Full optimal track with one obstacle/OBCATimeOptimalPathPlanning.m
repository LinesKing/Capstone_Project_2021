function [x] = OBCATimeOptimalPathPlanning(xInner, yInner,xOuter, yOuter, warm, initPoint, object, dMin)
...
for i = 1:N+1
    ...
    % [x, y, v, theta, a, omega]
    if (i == 1) % Satisfying initial conditions contraint
    options.lb = [options.lb, [initPoint(1) initPoint(2) vMin initPoint(3) aMin 0]];
...
x = reshape(sol(1:N*nStates), [nStates,N])';
...
% Initialise grad of objective function
...
for i = 1:N
    ...
    % Obstacle Constraints
    % A*([x(k);x(k+1)] - b)'*[x(k+6);x(k+7);x(k+8);x(k+9)];
    % norm(A'*[x(k+6);x(k+7);x(k+8);x(k+9)]);
...
% Define Constants
[N, n_states, ~, ~, ~, ~, x_outer, y_outer, x_inner, y_inner, A, b] = deal(auxdata{:});

% Number of constraints
n_constraints_triplet = 1;
n_constraints_system = 4;
n_constraints_object = 2;
n_constraints = n_constraints_triplet + n_constraints_system + n_constraints_object;

% Generate Jacobian (gradient of constraint functions)
J = spalloc(n_constraints*N,n_states*(N+1),32*N);
...
for i = 1:N
    ...
    J(m,k) = -1; J(m,k+2) = -x(k+10)*cos(x(k+3)); J(m,k+3) = x(k+10)*x(k+2)*sin(x(k+3)); J(m,k+n_states) = 1; J(m,k+10) = -x(k+2)*cos(x(k+3));
    J(m+1,k+1) = -1; J(m+1,k+2) = -x(k+10)*sin(x(k+3)); J(m+1,k+3) = -x(k+10)*x(k+2)*cos(x(k+3)); J(m+1,k+1+n_states) = 1; J(m+1,k+10) = -x(k+2)*sin(x(k+3)); 
    J(m+2,k+2) = -1; J(m+2,k+4) = -x(k+10); J(m+2,k+2+n_states) = 1; J(m+2,k+10) = -x(k+4);
    J(m+3,k+3) = -1; J(m+3,k+5) = -x(k+10); J(m+3,k+3+n_states) = 1; J(m+3,k+10) = -x(k+5);
    m = m + n_constraints_system;
    
    % Jacobian of Sticking to Triplet Constraint
    if x_inner(i) ~= x_outer(i)
        J(m,k) = -(y_outer(i)-y_inner(i))/(x_outer(i)-x_inner(i));
        J(m,k+1) = 1;
    end
    m = m + n_constraints_triplet;
...
J = zeros(n_constraints*N,n_states*(N+1));
...
for i = 1:N
    ...
    % Jacobian of System Dynamics
    J(m,k) = 1; J(m,k+2) = 1; J(m,k+3) = 1; J(m,k+n_states) = 1; J(m,k+10) = 1;
    J(m+1,k+1) = 1; J(m+1,k+2) = 1; J(m+1,k+3) = 1; J(m+1,k+1+n_states) = 1; J(m+1,k+10) = 1;
    J(m+2,k+2) = 1; J(m+2,k+4) = 1; J(m+2,k+2+n_states) = 1; J(m+2,k+10) = 1;
    J(m+3,k+3) = 1; J(m+3,k+5) = 1; J(m+3,k+3+n_states) = 1; J(m+3,k+10) = 1;
    m = m + n_constraints_system;   
...
% Convert Jacobian into sparse (MUST DO FOR IPOPT)
    J_struct = sparse(J);